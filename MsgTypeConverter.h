...
template<Msg::MsgType U, Msg::MsgType V>
class MsgTypeConverter : public SimpleBlock, public Sink, public Source {
public:
...
private:
    ...
    std::shared_ptr<Msg> convert(const std::shared_ptr<const Msg> &msg) {
        static_assert((U == Msg::RAW && (V == Msg::RTP_PACKET || V == Msg::RTCP_PACKET)) || (V == Msg::RAW && (U == Msg::RTP_PACKET || U == Msg::RTCP_PACKET)), "non-specialized template is for RAW to PACKET types and vice-versa");
